...
private val customBuildSuffix = Regex("-(?:sakura|miku)\\.\\d{8}\\.\\d+(?:-[A-Za-z0-9.-]+)?$")

internal fun isCustomBuild(versionName: String): Boolean =
    customBuildSuffix.find(versionName)?.range?.first?.let { it > 0 } == true

/** Our build marker must not make an unchanged official release look newer. */
internal fun officialComparisonVersion(versionName: String): Version =
    Version(if (isCustomBuild(versionName)) versionName.replace(customBuildSuffix, "") else versionName)
...
class UpdateChecker(
                ...
                data = try {
                    val response = client.newCall(
                        Request.Builder()
                            ...
                            .addHeader(
                                "User-Agent",
                                "MikuHub ${BuildConfig.VERSION_NAME} #${BuildConfig.VERSION_CODE}"
                            )
                            .build()
                    ).await()
    ...
    suspend fun downloadUpdate(download: UpdateDownload, manager: DownloadInstallManager): Long =
        manager.enqueueApk(download.url, download.name)
}
